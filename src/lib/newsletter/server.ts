...
export async function processNewsletterQueue() {
  ...
  for (const campaign of campaigns ?? []) {
    campaignResults.push({ id: campaign.id, ...(await dispatchCampaign(campaign.id)) });
  }
  ...
  for (const event of events ?? []) {
    await client.from("tenant_newsletter_trigger_events").update({ status: "processing" }).eq("id", event.id).eq("status", "pending");
    try {
      ...
      if (event.subscriber_id) {
        const subscriberResult = await client
          ...
          .maybeSingle();
        if (subscriberResult.data) subscriberRows = [subscriberResult.data];
        ...
        const subscribersResult = await client
          .from("tenant_newsletter_subscribers")
          .select("*")
          .eq("tenant_id", event.tenant_id)
          .eq("status", "active");
        subscriberRows = subscribersResult.data ?? [];
      }
      let nextAvailableAt: number | null = null;

      if (subscriberRows.length > 0) {
        for (const rawAutomation of automations ?? []) {
          const automation = messageFromRow(rawAutomation);
          const dueAt = new Date(event.created_at).getTime() + automation.delayMinutes * 60000;
          if (dueAt > Date.now()) {
            nextAvailableAt = nextAvailableAt === null ? dueAt : Math.min(nextAvailableAt, dueAt);
            continue;
          }
          for (const subscriberRow of subscriberRows) {
            const result = await sendDelivery({
              message: automation,
              subscriber: subscriberFromRow(subscriberRow),
              triggerEventId: event.id,
            });
            if (result.sent) automationsSent += 1;
          }
        }
      }
      await client.from("tenant_newsletter_trigger_events").update(
  ...
  return { campaigns: campaignResults, automationsSent };
}
