...
export function buildOrderConfirmationEmail(p: OrderConfirmationParams): string {
    ...
    .map(
      ...
      <tr>
        <td style="padding:10px 0;border-bottom:1px solid #eee;color:${brand.text};font-size:14px;">
          <strong>${l.qty} × ${escapeHtml(l.name)}</strong>${
            l.variantLabel ? ` <span style="color:${brand.muted};">(${escapeHtml(l.variantLabel)})</span>` : ""
          }
