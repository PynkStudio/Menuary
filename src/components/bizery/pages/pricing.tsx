...
import { PRICING_PLANS, annualSaving, type PricingPlan } from "@/lib/platform-pricing";
import { FAQSection, FinalCTASection } from "@/components/marketing/marketing-sections";

// Tabella confronto funzioni — adattata per aziende di servizi
const COMPARE_ROWS: { label: string; vetrina: boolean | string; operativita: boolean | string; autopilota: boolean | string }[] = [
  { label: "Sito aziendale su misura", vetrina: true,  operativita: true,  autopilota: true  },
  { label: "Listino servizi digitale",  vetrina: true,  operativita: true,  autopilota: true  },
  { label: "Hosting, SSL, backup",      vetrina: true,  operativita: true,  autopilota: true  },
  { label: "Prenotazioni / appuntamenti", vetrina: false, operativita: true,  autopilota: true  },
  { label: "Slot disponibilità",        vetrina: false, operativita: true,  autopilota: true  },
  { label: "CRM clienti",               vetrina: false, operativita: true,  autopilota: true  },
  { label: "Analytics e report",        vetrina: false, operativita: true,  autopilota: true  },
  { label: "Costi e margini",           vetrina: false, operativita: true,  autopilota: true  },
  { label: "Ruoli e permessi staff",    vetrina: false, operativita: true,  autopilota: true  },
  { label: "Multi-sede",                vetrina: false, operativita: true,  autopilota: true  },
  ...
  { label: "Supporto", vetrina: "Standard", operativita: "Prioritario", autopilota: "Dedicato" },
];

const PRICING_FAQ = [
  {
    q: "Il contratto è annuale?",
    a: "Sì. Il contratto ha durata annuale e non è previsto il recesso anticipato. Puoi però cambiare piano in qualsiasi momento: la variazione è attiva dal mese successivo.",
  },
  {
    q: "Cosa cambia tra fatturazione annuale e mensile?",
    a: "Con fatturazione annuale paghi un unico importo per i 12 mesi e benefici di un canone mensile ridotto. Con fatturazione mensile la spesa è ripartita mese per mese, ma il canone è più alto.",
  },
  {
    q: "Cos'è il costo di attivazione?",
    a: "È una tariffa una tantum per la configurazione iniziale: setup tecnico, onboarding, personalizzazione del sito e dei moduli. Non è inclusa nel canone e viene concordata prima dell'avvio.",
  },
  {
  ...
  },
  {
    ...
    a: "Sì, in qualsiasi momento. Aggiungere prenotazioni, CRM, analytics o multi-sede è una semplice attivazione, senza rifare il sito. La variazione è attiva dal mese successivo.",
  },
  {
  ...
  },
  {
    ...
    a: "Per il piano Vetrina, in media 3–5 settimane dalla prima chiamata. Il piano Operatività richiede 4–6 settimane per includere agenda, CRM e configurazione dei moduli operativi.",
  },
  {
...
export function BizeryPricingPage() {
  ...
  return (
    <BizeryShell>
        ...
        <div className="menuary-container pt-20 pb-20 lg:pt-28 lg:pb-24">
          <div className="grid items-end gap-14 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="menuary-fade-up">
              ...
              <h1
                className="mt-7 text-[clamp(3rem,6.8vw,6rem)] font-medium leading-[1.05] tracking-[-0.02em]"
      ...
      <section>
        <div className="menuary-container py-20 lg:py-24">
          ...
          <p className="mt-10 text-center text-xs uppercase tracking-[0.18em] text-[var(--menuary-muted)]">
            ...
            {billing === "annual" ? " · fatturazione annuale" : " · fatturazione mensile"}
          </p>
        </div>
      </section>
          ...
          <div className="mx-auto max-w-3xl">
            ...
            <div className="grid gap-8 sm:grid-cols-3 text-[14px] leading-7 text-[var(--menuary-muted)]">
              <div>
                ...
                <p>La durata minima è di 12 mesi. Non è previsto il recesso anticipato. Puoi cambiare piano in qualsiasi momento.</p>
              </div>
              <div>
                <p className="mb-2 font-semibold text-[var(--menuary-ink)]">Costo di attivazione</p>
                <p>Una tantum, concordata prima dell&apos;avvio. Copre setup tecnico, onboarding e personalizzazione. Non è inclusa nel canone.</p>
              </div>
              <div>
                <p className="mb-2 font-semibold text-[var(--menuary-ink)]">IVA e prezzi</p>
                <p>Tutti i prezzi esposti sono al netto dell&apos;IVA applicabile. La fattura sarà emessa con l&apos;aliquota vigente.</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <table className="w-full min-w-[42rem] border-collapse text-left text-[15px]">
                <thead>
                  <tr className="border-b border-[var(--menuary-ink)]">
                    ...
                    <th className="py-4 px-4 text-center font-semibold text-[var(--menuary-copper)]">Operatività</th>
                ...
                <tbody>
                  {COMPARE_ROWS.map((row) => (
                    <tr key={row.label} className="border-b border-[var(--menuary-line)]">
                      ...
                      <td className="py-4 px-4 text-center"><CellMark value={row.vetrina} /></td>
                      <td className="py-4 px-4 text-center"><CellMark value={row.operativita} /></td>
                      <td className="py-4 pl-4 text-center"><CellMark value={row.autopilota} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <FAQSection items={PRICING_FAQ} title="Tutto quello che vuoi sapere prima di iniziare." />
      <FinalCTASection />
    </BizeryShell>
  );
}
    ...
    >
      {plan.is_featured && (
        <span className="absolute -top-3 left-8 inline-flex items-center gap-1 rounded-full bg-[var(--menuary-copper)] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.22em] text-white">
          <Sparkles size={11} strokeWidth={2} /> Consigliato
        </span>
      )}
      <div>
        <h2
        ...
        <p className="mt-1 text-xs uppercase tracking-[0.22em] text-[var(--menuary-copper)]">{plan.tagline}</p>
      </div>
      <div>
        ...
        {billing === "annual" ? (
          <p className="mt-2 text-xs text-[var(--menuary-muted)]">
            <span className="font-semibold text-[var(--menuary-sage)]">Risparmi €{saving}/anno</span>{" "}rispetto al mensile
          </p>
        ) : (
          <p className="mt-2 text-xs text-[var(--menuary-muted)]">
...
function CellMark({ value }: { value: boolean | string }) {
  ...
  return <span className="text-[14px] font-semibold text-[var(--menuary-ink)]">{value}</span>;
}
