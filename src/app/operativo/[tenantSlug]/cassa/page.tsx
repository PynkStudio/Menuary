...
async function fetchPendingOrders(tenantSlug: string, locationId: string | null): Promise<PendingOrder[]> {
  const svc = createSupabaseServiceClient();
  if (!svc) return [];
  let q = svc
    .from("orders")
...
async function fetchCassa(tenantSlug: string, locationId: string | null): Promise<{ session: Session | null; movements: Movement[]; recent: Session[] }> {
  const svc = createSupabaseServiceClient();
  ...
  let openQ = svc
    ...
    .eq("tenant_id", tenantSlug)
    .eq("status", "open");
  if (locationId) openQ = openQ.eq("location_id", locationId);
  const { data: openRow } = await openQ
    .maybeSingle();
  ...
  let recentQ = svc
    ...
    .eq("tenant_id", tenantSlug)
    .eq("status", "closed");
  if (locationId) recentQ = recentQ.eq("location_id", locationId);
...
export default async function CassaPage({
    ...
    return (
      <div className="ga-dashboard">
        <header>
          ...
          <p className="ga-lead">Ogni sede ha la propria cassa, sessioni e ordini da incassare.</p>
        </header>
  ...
  const [data, pendingOrders] = auth.isDemo
    ...
    : await Promise.all([fetchCassa(tenantSlug, locationId), fetchPendingOrders(tenantSlug, locationId)]);
      ...
      {pendingOrders.length > 0 && (
        <section className="ga-card" style={{ borderColor: "var(--ga-warn, #B8332E)", borderWidth: 2 }}>
          <div className="ga-section-head">
            ...
            <Link href={`/operativo/${tenantSlug}/ordini?f=pending${locationSlug ? `&loc=${locationSlug}` : ""}`} className="ga-section-hint">
              Vedi tutti
            </Link>
          </div>
