 ...
 * Crea un nuovo tenant. Food e services includono la prima sede via RPC;
 * i tenant creative di Orpheo non usano sedi.
 * Body:
 ...
 *
 * Invariante: solo i tenant food/services nascono con almeno una sede.
 */
export async function POST(request: NextRequest) {
  ...
  if (vertical !== "creative" && (!address || !address.trim())) {
  ...
  if (vertical === "creative") {
    const { data, error } = await db
      ...
      .insert({
        ...
        status: status ?? "trial",
        domains: domains ?? [],
        preview_slug: previewSlug ?? tenantId,
        enabled: true,
        theme,
        features: { ...features, multiLocation: false },
        hours: [],
      } as never)
      .select("id")
      .single();
    ...
    createdTenantId = row?.tenant_id ?? tenantId;
