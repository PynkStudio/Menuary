...
import { buildAutoSignature, type AutoSignatureProfile } from "@/lib/email/signature-queries";
...
async function loadComposerProfile(userId: string): Promise<AutoSignatureProfile | null> {
  ...
  if (!data || !COMPOSE_ROLES.has(data.role as string)) return null;
  return data as AutoSignatureProfile;
}
